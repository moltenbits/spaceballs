...
struct WindowCommand: ParsableCommand {
  ...
  @Argument(help: "The window ID to activate (from 'spaceballs list')")
  var windowID: Int
  ...
  func run() throws {
    ...
    if AppBundleHelper.isRunningInAppBundle() {
      ...
      guard let appPath = AppBundleHelper.findInstalledAppBundle() else {
        fputs(
          "Error: Window activation requires either the Spaceballs GUI to be running,\n",
          stderr)
        fputs(
          "or the Spaceballs-CLI.app bundle installed (run 'make install').\n",
          stderr)
  ...
  private func delegateToRunningGUI() throws -> Bool {
    ...
    let replyName = "com.moltenbits.spaceballs.cli.reply.\(UUID().uuidString)"
    ...
    center.postNotificationName(
      Notification.Name("com.moltenbits.spaceballs.cli.activate"),
      object: nil,
